...
void								CmdManager::cmdCreateRoom(ServerClient *client, BasicCmd *msgClient)
{
	...
	sendUpdateRoom(client->getCurrentRoom());
	_mutex->unlock();
	std::cout << std::endl;
}
...
void			CmdManager::cmdLeaveRoom(ServerClient *client, BasicCmd *msgClient)
{
	...
	if (client->getCurrentRoom() != -1)
	{
		...
		int roomId = client->getCurrentRoom();
...
void			CmdManager::cmdSetStatus(ServerClient *client, BasicCmd *msgClient)
{
	...
	else
	{
		...
		std::cout << "Status changed : " << client->isReady() << std::endl;
		_mutex->lock();
		sendUpdateRoom(client->getCurrentRoom());
		_mutex->unlock();
	}
...
void			CmdManager::cmdQuitGame(ServerClient *client, BasicCmd *msgClient)
{
	BasicCmd	reply;
	char		*msgSerialized;
	...
	reply.setCommandType(REPLY_CODE);
	_mutex->lock();
	if (client->getCurrentRoom() == -1 || _roomManager->getRoomById(client->getCurrentRoom())->isReady() == false)
	{
		_mutex->unlock();
		reply.setCommandArg(std::to_string(NOT_IN_ROOM));
...
void										CmdManager::sendUpdateRoom(const int roomId)
{
	...
	try
	{
		vectClients = _roomManager->getRoomById(roomId)->getClients();
	}
...
void										CmdManager::sendEndGame(const Room &room)
{
	...
	while (it != vectClients.end())
	{
		_clientManager->addDataToSendTCP((*it)->getTCPSocket(), msgSerialized, sizeof(endCmd));
		(*it)->setStatus(false);
		it++;
	}
}